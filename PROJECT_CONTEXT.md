...
### Phase 11: Watchtower 2.0 (Robinhood Parity)
**Purpose**: Redesign the Watchtower to mimic the "Robinhood" options dashboard. Shift from a static data table to a P&L-focused card layout that emphasizes Total Return, Market Value, and Risk (Greeks).

| ID | Title | Acceptance Criteria (Pass/Fail) | Owners | Status |
|---|---|---|---|---|
| **A-P11-01** | **Backend P&L Engine** | **Logic Update**: Update `watchman.py` to calculate financial metrics during the heartbeat cycle. <br> **New Fields**: Calculate and persist `market_value` (Mark × 100 × Contracts), `unrealized_pnl` (Difference between Entry & Mark), and `return_pct`. <br> **Short Logic**: Ensure P&L handles "Short" logic correctly (Profit = Entry > Mark). | Arch | ✅ Completed |
| **A-P11-02** | **Live Greeks Ingestion** | **Data Update**: Update `MarketDataProvider` to fetch live Delta, Theta, and Gamma for active positions during the Watchman cycle. <br> **Schema**: Add `greeks` JSONB column to `ActivePosition` to store these values (e.g., `{"delta": -0.21, "theta": 0.05}`). | Quant | ✅ Completed |
| **A-P11-03** | **Watchtower UI Redesign** | **Visual Overhaul**: Replace the HTML Table in `/watchtower` with a "List of Cards" layout. <br> **Row Layout**: <br> 1. **Left**: Ticker (Bold) + Strategy (e.g., "AMD $150 Put"). <br> 2. **Right**: Market Value (Large) + Total Return % (Color-coded Green/Red). <br> **Typography**: Use standard Sans for labels and Monospace for all financial data. | Frontend | ✅ Completed |
| **A-P11-04** | **Portfolio Summary Header** | **New Component**: Add a "Portfolio Value" widget at the top of the Watchtower. <br> **Metrics**: Display `Total Market Value`, `Total Daily P&L` (Sum of all positions), and `Total Unrealized P&L`. <br> **Style**: Large, hero-style typography with an equity curve sparkline (optional placeholder). | Frontend | ✅ Completed |
| **A-P11-05** | **Position Detail Expansion** | **Interaction**: Clicking a position card expands to show "Robinhood-style" stats grid: <br> - **Avg Cost** vs **Mark Price** <br> - **Today's Return** vs **Total Return** <br> - **Portfolio Diversity** (% of total allocation) <br> - **Greeks** (Delta, Theta). | Frontend | ✅ Completed |

**Phase 11 implementation**: **A-P11-01**: `watchman.py` computes and persists `market_value` (Mark × 100 × Contracts), `unrealized_pnl` (Short: (Entry − Mark) × 100 × Contracts), and `return_pct` during each cycle. **A-P11-02**: `MarketDataProvider.get_greeks_for_position(ticker, strike, expiry_date, strategy)` added; Mock returns sample Delta/Theta/Gamma; `active_positions.greeks` JSONB populated in Watchman. Migration `005_watchtower_pnl_greeks` adds columns `market_value`, `unrealized_pnl`, `return_pct`, `greeks`. **A-P11-03**: `/watchtower` table replaced with card list (`flex flex-col gap-2`); each card shows Ticker (bold) + Strategy label, Market Value and Total Return % (monospace, green/red). **A-P11-04**: Portfolio Value widget at top with Total Market Value, Total Daily P&L (placeholder "—" until prior-day mark stored), Total Unrealized P&L, and optional sparkline placeholder. **A-P11-05**: Expandable card detail shows Avg Cost, Mark Price, Today's Return (placeholder), Total Return, Portfolio Diversity %, and Greeks (Delta, Theta, Gamma). `GET /positions` response includes `market_value`, `unrealized_pnl`, `return_pct`, `greeks`.


**Purpose**: Improve developer experience and user awareness regarding market hours and system states.
| ID | Title | Acceptance Criteria (Pass/Fail) | Owners | Status |
|---|---|---|---|---|
| **A-OPS-06** | **Force Market Updates & Status Banner** | **Backend Config**: Add `FORCE_MARKET_UPDATES` (bool, default `False`) to `app.config.Settings` (load from `.env`). <br> **Logic Update**: Modify `_watchman_job` in `main.py`. The guard clause must read: `if not is_market_hours() and not settings.FORCE_MARKET_UPDATES: return`. <br> **Heartbeat API**: Update `GET /heartbeat` response to include a `market_status` field returning `"OPEN"`, `"CLOSED"`, or `"FORCED"`. <br> **Frontend Banner**: Add a global alert in `AppShell` or `Dashboard`. <br> - If `CLOSED`: Show amber/blue banner "Markets are closed. Real-time updates paused." <br> - If `FORCED`: Show purple banner "Dev Mode: Market updates are being forced." | Arch, Frontend | ✅ Completed |

**A-OPS-06 implementation**: `config.Settings` has `force_market_updates` (default `False`, env `FORCE_MARKET_UPDATES`). `_watchman_job` guard is `if not is_market_hours() and not getattr(settings, "force_market_updates", False): return`. `GET /heartbeat` returns `market_status`: `"OPEN"` when market is open, `"FORCED"` when `force_market_updates` is True and market closed, `"CLOSED"` otherwise. AppShell fetches heartbeat and shows an amber banner when `CLOSED` and a purple banner when `FORCED`.

| **A-OPS-07** | **Configurable Watchman Interval** | **Backend Config**: Add `WATCHMAN_INTERVAL_MINUTES` (int, default `15`) to `app.config.Settings`. <br> **Logic Update**: Update `_schedule_watchman` in `main.py` to use `settings.watchman_interval_minutes` for the job interval instead of the hardcoded `15`. <br> **Validation**: Verify the scheduler respects the `.env` value (e.g., set to 5) and defaults to 15 if missing. | Arch | ✅ Completed |

**A-OPS-07 implementation**: `config.Settings` has `watchman_interval_minutes` (default `15`, env `WATCHMAN_INTERVAL_MINUTES`). `_schedule_watchman` uses `getattr(settings, "watchman_interval_minutes", 15)` for the APScheduler interval; scheduler respects `.env` and defaults to 15 if unset.

| **A-FIX-16** | **Implement Polygon Provider** | **Dependencies**: Add `polygon-api-client` to `requirements.txt`. <br> **Logic**: Implement `get_quote` in `PolygonMarketDataProvider`. <br> - Fetch `Last Trade` for the Option Contract (Mark). <br> - Fetch `Last Trade` for the Underlying Equity (Price). <br> **Fallback**: If no trade occurred today (weekend), use the `close` field from the previous session. | Dev | ✅ Completed |

**A-FIX-16 implementation**: `polygon-api-client` was already in `requirements.txt`. `PolygonMarketDataProvider.get_quote` implemented using Polygon REST API (requests): fetches last trade for underlying and for option (option ticker built from underlying + expiry + strategy + strike); falls back to previous session close (`/v2/aggs/ticker/{ticker}/prev`) when last trade is missing. `get_quote` interface extended with optional `strike`, `expiry_date`, `strategy`; Watchman passes `entry_data` into `get_mark_price_for_position` so Polygon receives option context.

### **Technical Notes for Implementation**
* **Backend (<code>watchman.py</code>)**: You currently fetch `mark_price`. You must now use this to derive `unrealized_pnl` before updating the DB.
    * *Formula (Short)*: `(Entry Price - Mark Price) * 100 * Contracts`
* **Frontend (<code>page.tsx</code>)**: The current `&lt;table className="w-full">` will need to be replaced with a `&lt;div className="flex flex-col gap-2">` to support the card layout.
* **Schema (<code>schemas.py</code>)**: The `Position` type definition in the frontend and the backend Pydantic model will need the new fields: `market_value`, `unrealized_pnl`, `return_pct`, and `greeks`.

| **A-OPS-08** | **Debug Logging Mode** | **Backend Config**: Add `BACKEND_DEBUG` (bool, default `False`) to `app.config.Settings`. <br> **Startup Logic**: Update `main.py` startup event to configure `logging.basicConfig`. If `settings.backend_debug` is True, set Level to `DEBUG`; else `INFO`. <br> **Instrumentation**: Add `logger.debug()` statements to `_watchman_job` (e.g., "Starting cycle") and `watchman.py` (e.g., "Checking ticker X", "Price updated to Y") to trace execution flow. | Arch | ✅ Completed |

**A-OPS-08 implementation**: `config.Settings` has `backend_debug` (default `False`, env `BACKEND_DEBUG`). Startup in `main.py` calls `logging.basicConfig(level=DEBUG if backend_debug else INFO, format=...)`. `_watchman_job` logs `"Starting Watchman cycle"` at DEBUG. `watchman.py` uses `logger = logging.getLogger("watchman")` and logs DEBUG: `"Checking ticker X (position_id=...)"`, `"Skipping X: quote not available"` on DataFetchError, and `"Price updated for X: mark=... underlying=..."` after updating last_heartbeat.
